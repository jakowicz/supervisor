...
SECRET_ENV_KEYS = {"LLM_API_KEY", "OPENAI_API_KEY", "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY"}
...
def _write_env(path: Path, values: dict[str, str]) -> None:
    """Write versioned configuration separately from private credentials."""
    ...
    for key in sorted(set(values) - SECRET_ENV_KEYS):
        value = values[key]
        if value:
    ...
    secrets = ["# Private credentials for Supervisor. Never commit this file.", ""]
    for key in sorted(SECRET_ENV_KEYS):
        value = values.get(key, "")
        if value:
            secrets.append(f"{key}={value}")
    secrets_path = path.with_name(".secrets.env")
    if len(secrets) > 2 or secrets_path.exists():
...
def configure(path: Path) -> None:
    existing = dotenv_values(path) if path.is_file() else {}
    secrets = dotenv_values(path.with_name(".secrets.env")) if path.with_name(".secrets.env").is_file() else {}
    existing = {**existing, **secrets}
    ...
    print(f"Wrote {path} and private credentials in {path.with_name('.secrets.env')}. Secrets were not printed.")
...
def initialise_project(
    ...
    *,
    ...
    _append_gitignore(project_root / ".gitignore", "/.secrets.env")
    ...
    secrets_example = project_root / "supervisor" / ".secrets.env.example"
    if secrets_example.is_file():
        shutil.copyfile(secrets_example, project_root / ".secrets.env.example")
    if config.exists():
